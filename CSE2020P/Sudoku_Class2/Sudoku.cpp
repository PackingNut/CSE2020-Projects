...
//test sudoku table to see if it follows sudoku rules 
bool test_sudoku(const Vector<Vector<int> > &sud)
{
    assert(sud.size() == 6 && sud[0].size() == 6);
    for (int i = 0; i < 6; i++)
        if (!row_1to6(i, sud)) 
        //check if each row has numbers 1-6
            return false;
    for (int i = 0; i < 6; i++)
        if (!col_1to6(i, sud))
        //check if each column has numbers 1-6
            return false;
    for (int i = 0; i < 6; i++)
        if (!rect_1to6(i, sud))
        //check if each 3x2 rectangle has numbers 1-6
            return false;
    return true;
...
//find the missing numbers and put them in pairs (row, col)
Vector<pair<int, int> > find_empty(const Vector<Vector<int> >& sud)
{
    ...
    for(int i = 0; i < 6; i++)
    //used to search through each row and column
    //i is the row and j is the column
    {
        for(int j = 0; j < 6; j++)
        {
            ...
            if (sud[i][j] == 0)
            //finds if there is a 0 in any column or row
            {
                coord.first = i; //access the row
                coord.second = j; //access the column
                z_cells.push_back(coord); //store the above values inside z_cells
    ...
    return z_cells;
    //prints out what values were missing in the program

}

//Our triple nested for loop madness lol
//checks each pair and equals them to single variables to 
//to add to the broken sudoku
...
int fixSudo(Vector<Vector<int> >& sud, const Vector<pair<int, int> >& z_cells)
{
    for(int i = 1; i <= 6; i++)
    {
        ...
        //read out first coordinate
        for(int j = 1; j <= 6; j++)
        {
            sud[z_cells[1].first][z_cells[1].second] = j;
            //read out second coordinate
            for(int k = 1; k < 6; k++)
            //checks 1 through 6 sections
            {
                sud[z_cells[2].first][z_cells[2].second] = k;
                //read out last coordinate
                if(test_sudoku(sud))
                //test if the sudoku is correct after being fixed
                {
                    return 1;
                    //return true
                }
    ...
    return 0; 
    //returns false or "0" if there was no solution that could be found
