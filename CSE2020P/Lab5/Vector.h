...
class Vector
{
    public: 
        ...
        {
            assert(index >= 0 && index < theSize);
            //ensure the index value is in within the proper bounds
            ...
            //pointer to the memory location right after the last element
        ...
        void erase(int index, int& ops)
        {
            ...
            {
                data[i] = data[i +1];
                ops++; //count up 1 for constant block of comp.
            }
            pop_back();
            return;
        }
